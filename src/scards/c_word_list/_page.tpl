    ...
    <div class="{%$cardName%}">
        {%foreach $tplData.words as $key=>$item%}
            ...
            <div class="{%$cardName%}-item flex flex-justify">
                ...
                {%$url = "http://bj.qingpai.baidu.com/bdbox/gsword/word/"|cat:urlencode($item.keyword)|cat:"/"%}
                {%$redirectType = "bdbox_bingo_keyword_"|cat:$item.type%}
                {%*$redirectUrl = "/redirect?u="|cat:urlencode($url)|cat:"&fr="|cat:urlencode($redirectType)|cat:"&o="|cat:urlencode($tplData.order)*%}
                {%$redirectUrl = $url%}
